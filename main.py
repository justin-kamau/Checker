...
def get_current_directors(number):
    ...
    for item in data['items']:
        if item.get('resigned_on') is None:
            ...
            # Skip secretaries entirely
            if 'secretary' in officer_role:
                continue
                ...
                for match_idx, match_info in enumerate(matches):
                    ...
                    with col2:
                        ...
                        if match_key not in st.session_state.match_decisions:
                            default_decision = "Yes" if similarity >= 0.85 else "No"
                        ...
                        decision = st.radio(
                            ...
                            key=match_key,
                            horizontal=True,
                            index=0 if st.session_state.match_decisions[match_key]['decision'] == "Yes" else 1
                        )
